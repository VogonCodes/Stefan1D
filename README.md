...
$$ \frac{L\sqrt{\pi}}{c(T_{\mathrm{melt}}-T_{\mathrm{top}})} = \frac{\exp(-\lambda^{2})}{\lambda\erf\lambda} $$
...
I just put that into for loop and I don't regret it.
...
\end{pmatrix}
\begin{pmatrix}
    T^{n+1}_{1}\\T^{n+1}_{2}\\T^{n+1}_{3}\\\vdots\\T^{n+1}_{J-1}
\end{pmatrix}=
\begin{pmatrix}
...
$$
\begin{aligned}
    f &= \frac{\exp(-\lambda^{2})}{\lambda\erf\lambda} - \frac{L\sqrt\pi}{c\Delta T}\\
    f' &= -\frac{\exp(-\lambda^{2}}{\lambda^{2}\erf\lambda}-\frac{2\exp(-\lambda^{2})}{\sqrt{\pi}\lambda\erf^{2}\lambda}
\end{aligned}
$$
Thus,
...
$$ T_{j}^{n} = T_{\mathrm{top}} + \Delta T \frac{\erf{\eta_{j}^{n})}{\erf\lambda}\, $$
